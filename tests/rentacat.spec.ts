 ...
 * 2. The URL 'http://localhost:8080' is open on the web browser.
...
test('TEST-1-RESET', async ({ page }) => {
    ...
    await expect(listing.locator('#cat-id1')).toHaveText('ID 1. Jennyanydots');
    await expect(listing.locator('#cat-id2')).toHaveText('ID 2. Old Deuteronomy');
    await expect(listing.locator('#cat-id3')).toHaveText('ID 3. Mistoffelees');
});
/**
...
test('TEST-2-CATALOG', async ({ page }) => {
    ...
    const images = page.locator('ol').locator('img');
...
test('TEST-6-RETURN', async ({ page }) => {
    ...
    await expect(listing.locator('#cat-id1')).toHaveText('ID 1. Jennyanydots');
    await expect(listing.locator('#cat-id2')).toHaveText('ID 2. Old Deuteronomy');
    await expect(listing.locator('#cat-id3')).toHaveText('Rented out');
